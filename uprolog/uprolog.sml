...
datatype unit_test 
  ...
  | CHECK_SOLUTIONS     of goal list * (name * term) list list
...
fun lower "is" = RESERVED "is"
  ...
  | lower "check_solutions"     = RESERVED "check_solutions"
  | lower s    = LOWER s
(* type declarations for consistency checking *)
...
local
  ...
  fun int cs [] = intFromChars cs >>=+ (fn n => INT_TOKEN (implode cs))
    | int cs ids = 
...
val int    = (fn INT_TOKEN s => SOME (valOf (Int.fromString s)) | _ => NONE) <$>? token
...
fun checkSolutions goals =
  let fun split (gs', []) = OK (CHECK_SOLUTIONS (reverse gs', []))
        | split (gs', rest as ("cons", _) :: _) =
             validateList ([], rest) >>=+
             (fn substs => CHECK_SOLUTIONS (reverse gs', substs))
        | split (gs', rest as ("nil", _) :: _) =
             validateList ([], rest) >>=+
             (fn substs => CHECK_SOLUTIONS (reverse gs', substs))
        | split (gs', g :: gs) = split (g :: gs', gs)
      and validateList (thetas', (theta as ("cons", _)) :: rest) =
             validate ([], APPLY theta) >>=
             (fn theta' => validateList (theta' :: thetas', rest))
        | validateList (thetas', (theta as ("nil", _)) :: rest) =
             validate ([], APPLY theta) >>=
             (fn theta' => validateList (theta' :: thetas', rest))
        | validateList (theats', g :: gs) =
             ERROR ("in check_solutions, expected a substitution list but got " ^
                    goalString g)
        | validateList (thetas', []) = OK (reverse thetas')
      and validate (theta', APPLY ("cons", [APPLY ("=", [VAR x, t]), theta])) =
             validate ((x, t) :: theta', theta)
        | validate (theta', APPLY ("cons", [APPLY ("=", [t1, t2]), theta])) =
            ERROR ("in check_solutions, " ^ termString t1 ^ " is set to " ^
                   termString t2 ^ ", but " ^ termString t1 ^
                                                           " is not a variable")
        | validate (theta', APPLY ("cons", [t, theta])) =
            ERROR ("in check_solutions, expected a substitution but got " ^
                   termString t)
        | validate (theta', APPLY ("nil", [])) =
             OK (reverse theta')
        | validate (theta', t) =
             ERROR ("in check_solutions, expected a substitution list but got " ^
                    termString t)
...
val _ = op checkSolutions : goal list -> unit_test error
...
val unit_test =
 ...
 <|> reserved "check_solutions" *>
        (wrap "(" ")" (checkSolutions <$>! commas goal)
         <?> "check_solutions(goal, ... [, [X1 = t1, ...], ...])")
(* type declarations for consistency checking *)
            ...
            handle NotFound _ =>
              let fun search [] = fail ()
                    | search (clause :: clauses) =  
                        ...
                        in  solveMany (map (goalsubst theta) Hs)
                                      theta
                                      succ
                                      resume
                        end
...
val _ = op search : clause list -> 'a
      ...
      and solveMany []            theta succ fail = succ theta fail
        | solveMany (goal::goals) theta succ fail =
            solveOne goal
            (fn theta' => fn resume =>
               solveMany (map (goalsubst theta') goals)
                         (compose (theta', theta))
                         succ
                         resume)
            fail
(* type declarations for consistency checking *)
val _ = op solveOne  : goal ->
                       (subst -> (unit -> 'a) -> 'a) (* succ *) ->
                       (unit -> 'a) (* fail *) ->
                       'a
val _ = op solveMany : goal list -> subst ->
                       (subst -> (unit -> 'a) -> 'a) (* succ *) ->
                       (unit -> 'a) (* fail *) ->
                       'a
      fun q gs succ fail = solveMany gs idsbst succ fail
  in  q
  end
(* type declarations for consistency checking *)
val _ = op query : database -> goal list ->
                   (subst -> (unit -> 'a) -> 'a) (* succ *) ->
                   (unit -> 'a) (* fail *) ->
                   'a
(* interaction S81c *)
...
fun processDef (cq, database, interactivity) =
  let val () = resetComputationLimits ()     (* OMIT *)
      fun process (ADD_CLAUSE c) = addClause (c, database)
        | process (QUERY gs) = (
...
(* definitions of [[checkSatisfiedPasses]] and [[checkUnsatisfiablePasses]] S596a *)
      ...
      fun checkSolutionsPasses (gs, expSolns) =
        let fun solnString soln = 
               concat ["[",
                       String.concatWith ", " (map (fn (x, t) =>
                                                    concat [x, " = ", termString t])
                                                   soln),
                       "]"]
            fun solnsString solns =
               concat ["[",
                       String.concatWith ", " (map solnString solns),
                       "]"]

            fun termCompare (t1, t2) =
               case (t1, t2) of
                  (VAR n1, VAR n2) => String.compare (n1, n2)
                | (VAR _, _) => LESS
                | (_, VAR _) => GREATER
                | (LITERAL i1, LITERAL i2) => Int.compare (i1, i2)
                | (LITERAL _, _) => LESS
                | (_, LITERAL _) => GREATER
                | (APPLY (n1, ts1), APPLY (n2, ts2)) =>
                     (case String.compare (n1, n2) of
                         LESS => LESS
                       | EQUAL => List.collate termCompare (ts1, ts2)
                       | GREATER => GREATER)
            fun solnCompare (soln1, soln2) =
               List.collate (fn ((x1, t1), (x2, t2)) =>
                             case String.compare (x1, x2) of
                                LESS => LESS
                              | EQUAL => termCompare (t1, t2)
                              | GREATER => GREATER)
                            (soln1, soln2)
            fun solnsCompare (solns1, solns2) =
               List.collate solnCompare (solns1, solns2)
            ...
            fun solnSort (soln : (name * term) list) =
               isort (fn ((x1, _), (x2, _)) => String.compare (x1, x2)) soln
            fun solnsSort (solns : (name * term) list list) =
               isort solnCompare (map solnSort solns)

            val expSolns = solnsSort expSolns
            val actSolns = ref []
            fun succ soln fail =
               (actSolns := (stripSubst soln) :: !actSolns ;
                fail ())
            fun fail () =
               case solnsCompare (expSolns, solnsSort (!actSolns)) of
                  EQUAL => true
                | _ =>
                     failtest ["check_solutions failed: expected ", qstring gs,
                               " to give solutions ", solnsString expSolns,
                               ", but it gave solutions ", solnsString (!actSolns)]
        in  query database gs succ fail
            handle e => failtest ["check_solutions failed: proving ", qstring gs, " caused an error"]
        end
      ...
      fun passes (CHECK_UNSATISFIABLE gs)      = checkUnsatisfiablePasses gs
        ...
        | passes (CHECK_SOLUTIONS (gs, thetas)) = checkSolutionsPasses (gs, thetas)
(* type declarations for consistency checking *)
